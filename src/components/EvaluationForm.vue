...
export default {
  ...
  computed: {
    ...
    checklist() {
      if (this.case_details.case_type === 0)
        return this.$store.state.license.checklist;
      else if (this.case_details.case_type === 1)
        return this.$store.state.certificate.checklist;
      else return [];
    },
    recommended_tasks() {
      if (this.case_details.case_type === 0)
        ...
        return this.$store.state.certificate.recommended_tasks;
      else return [];
    },
    is_payment() {
  ...
  methods: {
    ...
    processLicense(app) {
      if (this.final_decision === 1) {
        ...
        return this.$upload(details, "DENIED");
      } else return null;
    },
    processCertificate(app) {
      if (this.final_decision === 1) {
        ...
        return this.$upload(certificate, "CERT");
      } else if (this.final_decision === 3) {
        // Disapproved
        var details = {
          date_created: this.formatDate(new Date()),
