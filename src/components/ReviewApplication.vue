    ...
    >
      <v-card>
        <v-toolbar class="title-bg headline white--text" dark>
          ...
          >Case No.: {{case_details.case_no}} ({{getAppType(case_details.application_type, case_details.case_type)}})</span>
                          ...
                          <v-layout row wrap>
                            <v-flex xs12 class="body-2">
                              ...
                              <v-chip
                                ...
                                style="float: right"
                                label
                                :color="getPaymentStatusColor(case_details.payment_status)"
                                text-color="white"
                              >{{getPaymentStatus(case_details.payment_status)}}</v-chip>
                            </v-flex>
                            ...
                            >
                              ...
                              <v-flex xs12 class="body-2">Transaction history</v-flex>
                              <v-flex
                                xs12
                                v-for="(item, index) in history_transactions"
                                :key="`e${index}`"
                              >
                                <span
                                  class="body-1"
                                >{{getModeOfPayments(item.payment_details.mode_of_payment)}}</span> -
                                <i class="caption">
                                  {{
                                ...
                                <span
                                  class="body-1"
                                  style="float: right"
                                >₱ {{numberWithCommas(item.payment_details.total_amount)}}</span>
                              </v-flex>
                              <v-flex xs12 class="body-2 font-weight-bold">
                                Total Amount Paid:
                                <span
                              ...
                              <v-flex xs12 class="body-2 font-weight-bold">
                                ...
                                <span
                                  style="float: right"
                                >₱ {{numberWithCommas(current_remaining_balance)}}</span>
                              </v-flex>
                            </template>
                          </v-layout>
                        </v-card-text>
        ...
        <v-card-actions v-if="for_unassigned">
          <v-btn color="primary" @click="show_confirmation=true" :loading="loading" block>
            CLAIM
...
export default {
  ...
  methods: {
    init() {
      console.log("this.case_details :", this.case_details);
      if (this.claimed) {
        if (this.is_payment) this.show_tabs(false, "show_tab2");
        else this.show_tabs(false, "show_tab1");
      } else this.show_tabs(true);
